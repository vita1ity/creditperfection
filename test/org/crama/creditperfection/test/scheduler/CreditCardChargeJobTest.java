...
public class CreditCardChargeJobTest {
	...
	@Ignore
	@Test
	public void testExpiredWithoutDiscount() {
		...
		List<Subscription> activeSubscriptions = Arrays.asList(
				new SubscriptionBuilder().status(SubscriptionStatus.TRIAL).discount(discounts.get(0)).build(),
				new SubscriptionBuilder().id(1l).status(SubscriptionStatus.TRIAL).discount(discounts.get(1)).build(),
				new SubscriptionBuilder().id(2l).discount(discounts.get(2)).build());
		...
		for (Subscription s: activeSubscriptions) {
			assertEquals(s.getDiscount().getDiscountStatus(), DiscountStatus.USED);
			Logger.info("Discount status: " + s.getDiscount().getDiscountStatus());
		}
		
		verify(subscriptionServiceMock, times(1)).findExcludingStatus(SubscriptionStatus.CANCELLED);
		verify(subscriptionServiceMock, times(activeSubscriptions.size())).checkExpired(any(Subscription.class));
		verify(discountServiceMock, times(activeSubscriptions.size())).checkDiscountExpired(any(Discount.class));
